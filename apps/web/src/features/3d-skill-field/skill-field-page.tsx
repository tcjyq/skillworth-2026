...
function SkillFieldExperience() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.stage} aria-labelledby="skill-field-title">
      ...
      <div className={styles.scopeRail} aria-label="数据范围"><div>{[0, 1].map((copyIndex) => <section key={copyIndex} aria-hidden={copyIndex === 1}><span>{globalData.job_count} 个岗位</span><span>{globalData.company_count} 家公司</span><span>{globalData.skill_count} 项观测技能</span><span>近 180 天</span><span>{globalData.source_role === "china_supplementary" ? "中国公开技术岗位补充样本" : globalData.source_role}</span></section>)}</div></div>
      <div className={styles.sceneCopy}>
        <h1 id="skill-field-title" className="max-w-5xl">{copy.title}</h1>
    ...
    <footer className={styles.footer}><p>{globalData.disclaimer}</p><Link href="/methodology">查看计算方法与证据边界</Link></footer>
  </main>;
}
