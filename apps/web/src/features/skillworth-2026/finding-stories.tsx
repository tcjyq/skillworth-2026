  ...
  return (
    <section className="border-b border-[var(--sw-line)]" aria-labelledby="findings-intro-title">
      <div className="mx-auto grid max-w-[1560px] gap-4 px-5 py-9 sm:px-8 md:grid-cols-[260px_1fr] lg:px-12">
        ...
        <p className="max-w-[820px] text-pretty text-sm leading-7 text-[var(--sw-text-secondary)]">答案不是一张排行榜：先看学习效率前沿，再看需求与技值的背离；目标岗位会改变答案，技能组合有不同强度，而排名必须连同敏感性区间一起读。</p>
      </div>
    </section>
  );
}
  ...
  return (
    <div className="grid border-b border-[var(--sw-line)] lg:grid-cols-2" aria-label="Efficiency Frontier 与 Demand divergence">
      <article className="py-7 pr-0 lg:border-r lg:border-[var(--sw-line)] lg:pr-10">
        ...
        <p className="mt-3 max-w-[610px] text-xs leading-6 text-[var(--sw-muted)]">在通过候选门槛且排名稳健的具体技能中，三者组成当前 180d 的效率前沿。</p>
      ...
      <article className="py-7 lg:pl-10">
        ...
        <p className="mt-5 max-w-[610px] text-xs leading-6 text-[var(--sw-text-secondary)]">{findings.cpp.jobCount} 个岗位；在从零达到可用于初级岗位任务的 <strong className="font-mono font-medium text-[var(--sw-text)]">{findings.cpp.learningHours}h 预期学习投入假设</strong>下，市场信号会被学习投入折损。这不是“C++ 不值得学”，而是需求排名不等于学习投资优先级。</p>
      </article>
    </div>
  );
}
    ...
    <section className="cinematic-reveal border-y border-[var(--sw-line)]" aria-labelledby="role-story-title">
      <div className="mx-auto max-w-[1560px] px-5 py-16 sm:px-8 lg:px-12 lg:py-22">
        <div className="grid gap-4 md:grid-cols-[260px_1fr]">
          ...
          <p className="max-w-[720px] text-xs leading-6 text-[var(--sw-muted)]">同一套 SkillWorth 方法，换成目标岗位切片后，专业技术栈会越过全局通用技能。样本量必须与角色排名同时阅读。</p>
        </div>
        {findings ? <div className="mt-9 grid border-y border-[var(--sw-line)] lg:grid-cols-2">
          {findings.roles.map((role, index) => (
            <article key={role.role} className={`py-7 ${index === 0 ? "lg:border-r lg:border-[var(--sw-line)] lg:pr-10" : "border-t border-[var(--sw-line)] lg:border-t-0 lg:pl-10"}`}>
              ...
              <button onClick={() => onSelectRole(role.role === "DevOps" ? "devops_engineer" : "data_engineer")} className="sw-focus mt-5 inline-flex items-center gap-2 text-xs text-[var(--sw-accent)] hover:text-[#e0ed9b]">查看 {role.role} 排名 <ArrowRight size={14} /></button>
            </article>
          ))}
...
export function SynergyStory({ findings }: { findings: FinalFindings | null }) {
  return (
    <section className="cinematic-reveal mx-auto max-w-[1560px] px-5 py-16 sm:px-8 lg:px-12 lg:py-22" aria-labelledby="synergy-story-title">
      <div className="grid gap-4 md:grid-cols-[260px_1fr]">
        ...
        <p className="max-w-[760px] text-xs leading-6 text-[var(--sw-muted)]">Absolute cooccurrence 回答“共同出现了多少次”；Jaccard / PMI affinity 回答“相对各自出现频率，它们有多偏好一起出现”。两者不能混为同一个强度。</p>
      </div>
      ...
      {findings && <p className="mt-5 max-w-[920px] text-xs leading-6 text-[var(--sw-muted)]">网络分母为 <strong className="font-mono font-medium text-[var(--sw-text-secondary)]">{findings.synergy.sampleSize.toLocaleString("en-US")} all-active canonical jobs</strong>，不是 180d 独立网络。共现是关联，不是因果，也不表示这些技能必须一起学。</p>}
    </section>
  );
}
...
export function RobustCoreHeading({ findings }: { findings: FinalFindings | null }) {
  return (
    <div className="mb-8 grid gap-5 md:grid-cols-[260px_1fr]" aria-labelledby="picks-title">
      ...
      <div>
        <p className="max-w-[760px] text-xs leading-6 text-[var(--sw-muted)]">头部技能在既有权重情景下形成稳定核心；长尾必须读区间，而不是把当前单一名次当成确定事实。</p>
        ...
        <p className="mt-4 text-[11px] leading-5 text-[var(--sw-muted)]">90d / 365d / all-active 的窗口一致性只作为 supporting evidence；窗口重叠且不是 trend。</p>
      </div>
    </div>
  );
}
