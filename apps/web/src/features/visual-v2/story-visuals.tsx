...
export function CppMoment({ findings, metadata }: { findings: FinalFindings; metadata: ChinaSkillWorthResponse }) {
  ...
  return <div className={styles.cppMomentVisual}>
    ...
    <div className={styles.cppRanks}><div><span>招聘需求排名</span><strong>#{cpp.demandRank}</strong></div><div className={styles.cppTransition}><small>加入学习投入后</small><i aria-hidden="true">→</i></div><div><span>学习性价比排名</span><strong>#{cpp.skillworthRank}</strong></div></div>
